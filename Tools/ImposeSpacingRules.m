    ...
    for k = 1 : numel( files )
        ...
        for i = 1 : numel( lines )
            ...
            if strlength( strtrim( codePart ) ) > 0
                ...
                dqMatches = regexp( codePart, '"(?:[^"]|"")*"', 'match' );
                for j = 1 : numel( dqMatches )
                    codePart = replace( codePart, dqMatches( j ), sprintf( '__DQ_%d__', j));
                end

                % Protect single-quote strings (avoiding transpose operator)
                sqRegex = '(?<=[^a-zA-Z0-9_\)\]\}\.])''(?:[^'']|'''')*''';
                sqMatches = regexp( codePart, sqRegex, 'match' );
                for j = 1 : numel( sqMatches )
                    codePart = replace( codePart, sqMatches( j ), sprintf( '__SQ_%d__', j));
                end
                ...
                % Restore single-quote strings
                for j = 1 : numel( sqMatches )
                    codePart = replace( codePart, sprintf( '__SQ_%d__', j), sqMatches(j));
                end

                % Restore double-quote strings
                for j = 1 : numel( dqMatches )
                    codePart = replace( codePart, sprintf( '__DQ_%d__', j), dqMatches(j));
                end

                % Remove padding
                codePart = extractAfter( codePart, 1 );
            end
        ...
        writelines( cleanLines, filePath, 'LineEnding', '\n' );
    end
end
