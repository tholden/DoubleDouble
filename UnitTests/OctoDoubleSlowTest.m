classdef OctoDoubleSlowTest < matlab.unittest.TestCase

    % OctoDoubleSlowTest Test suite for OctoDoubleSlow class
    ...
    methods ( TestMethodSetup )
        ...
        function CreateTestData( TestCase )
            ...
            digits( 300 );
            digits( 300 );
            TestCase.SmallValues = OctoDoubleSlow( [ 1e-10, 2e-10, 3e-10 ] );
            TestCase.MediumValues = OctoDoubleSlow( [ 1, 2, 3 ] );
            TestCase.LargeValues = OctoDoubleSlow( [ 1e10, 2e10, 3e10 ] );
            TestCase.ComplexValues = OctoDoubleSlow( [ 1 + 1i, 2 + 2i, 3 + 3i ] );

            % Create matrix test data
            TestCase.MatrixValues = OctoDoubleSlow( [ 1, 2, 3; 4, 5, 6; 7, 8, 9 ] );
        end
    ...
    methods ( Test )
        ...
        function TestPlus( TestCase )
            A = OctoDoubleSlow( [ 1.5, -2.7, 3.1 ] ) ./ 7 + exp( OctoDoubleSlow( 0.1 ) );
            B = OctoDoubleSlow( [ 0.8, 5.5, -1.2 ] ) .* sqrt( OctoDoubleSlow( 2 ) );
            A_vpa = vpa( A );
            B_vpa = vpa( B );
        ...
        function TestMtimes( TestCase )
            A = OctoDoubleSlow( [ 1.5, -2.7; 3.1, 0.4 ] ) ./ 7 + exp( OctoDoubleSlow( 0.1 ) );
            B = OctoDoubleSlow( [ 0.8, 5.5; -1.2, 2.3 ] ) .* sqrt( OctoDoubleSlow( 2 ) );
            A_vpa = vpa( A );
            B_vpa = vpa( B );
        ...
        function TestLdivide( TestCase )
            A = OctoDoubleSlow( 2 );
            B = OctoDoubleSlow( [ 10, 20, 30 ] );
            C = A .\ B;
            TestCase.verifyEqual( double( C ), [ 5, 10, 15 ] );
        end

        function TestMldivide( TestCase )
            A = OctoDoubleSlow( [ 3, 1; 1, 2 ] );
            B = OctoDoubleSlow( [ 9; 8 ] );
            X = A \ B;
            Expected = [ 2; 3 ];
        ...
        function TestColon( TestCase )
            A = OctoDoubleSlow( 1 ) : OctoDoubleSlow( 5 );
            TestCase.verifyEqual( double( A ), 1 : 5 );

            B = OctoDoubleSlow( 1 ) : OctoDoubleSlow( 0.5 ) : OctoDoubleSlow( 3 );
            TestCase.verifyEqual( double( B ), 1 : 0.5 : 3 );
        end

        function TestConcatenation( TestCase )
            A = OctoDoubleSlow( [ 1, 2 ] );
            B = OctoDoubleSlow( [ 3, 4 ] );
        ...
        function TestRound( TestCase )
            A = OctoDoubleSlow( [ 1.4, 1.5, 2.5, -1.5 ] );
            R = round( A );
        ...
        function TestSqrt( TestCase )
            A = OctoDoubleSlow( [ 4, 9, 16 ] ) ./ 7 + exp( OctoDoubleSlow( 0.1 ) );
            S = sqrt( A );
            expected = sqrt( vpa( A ) );
        ...
        function TestSin( TestCase )
            A = [ 0, OctoDoubleSlow.pi / 6, OctoDoubleSlow.pi / 4, OctoDoubleSlow.pi / 3, OctoDoubleSlow.pi / 2 ];
            S = sin( A );
            expected = sin( vpa( A ) );
            abs_err = abs( vpa( S ) - expected );
            err = abs_err ./ max( 1, abs( expected ) );
        ...
        function TestTan( TestCase )
            A = [ 0, OctoDoubleSlow.pi / 6, OctoDoubleSlow.pi / 4, OctoDoubleSlow.pi / 3 ];
            T = tan( A );
            expected = tan( vpa( A ) );
        ...
        function TestAsin( TestCase )
            A = [ 0, 0.5, 1 / sqrt( OctoDoubleSlow( 2 ) ), sqrt( OctoDoubleSlow( 3 ) ) / 2, 1 ];
            As = asin( A );
            expected = asin( vpa( A ) );
        ...
        function TestAcos( TestCase )
            A = [ 1, sqrt( OctoDoubleSlow( 3 ) ) / 2, 1 / sqrt( OctoDoubleSlow( 2 ) ), 0.5, 0 ];
            Ac = acos( A );
            expected = acos( vpa( A ) );
        ...
        function TestAtan( TestCase )
            A = [ 0, 1 / sqrt( OctoDoubleSlow( 3 ) ), 1, sqrt( OctoDoubleSlow( 3 ) ) ];
            At = atan( A );
            expected = atan( vpa( A ) );
        ...
        function TestAtan2( TestCase )
            Y = OctoDoubleSlow( [ 0, 1, 1, 1 ] );
            X = [ 1, sqrt( OctoDoubleSlow( 3 ) ), 1, 1 / sqrt( OctoDoubleSlow( 3 ) ) ];
            A = atan2( Y, X );
        ...
        function TestMod( TestCase )
            A = OctoDoubleSlow( [ 7, 7, -7, -7 ] );
            B = OctoDoubleSlow( [ 3, -3, 3, -3 ] );
            M = mod( A, B );
        ...
        function TestLU( TestCase )
            A = OctoDoubleSlow( [ 2, -1, 0; -1, 2, -1; 0, -1, 2 ] );
            [ L, U, P ] = lu( A );
            PA = P * A;
            LU = L * U;
        ...
        function TestQR( TestCase )
            A = OctoDoubleSlow( [ 12, -51, 4; 6, 167, -68; -4, 24, -41 ] );
            [ Q, R ] = qr( A );
            QR = Q * R;
        ...
        function TestDet( TestCase )
            ...
            B = OctoDoubleSlow( [ 1, 2, 3; 4, 5, 6; 7, 8, 9 ] );
            D2 = det( B );
            expected2 = vpa( 0 );
        ...
        function TestChol( TestCase )
            A = OctoDoubleSlow( [ 4, 12, -16; 12, 37, -43; -16, -43, 98 ] );
            R = chol( A );
            RtR = R' * R;
        ...
        function TestMeshgrid( TestCase )
            X = OctoDoubleSlow( [ 1, 2, 3 ] );
            Y = OctoDoubleSlow( [ 4, 5 ] );
        ...
        function TestColonMixedTypes( TestCase )
            ...
            A = OctoDoubleSlow( 1 ) : 5;
            TestCase.verifyEqual( double( A ), 1 : 5 );
            TestCase.verifyTrue( isa( A, 'OctoDoubleSlow' ) );

            B = OctoDoubleSlow( 1 ) : 0.5 : 3;
            TestCase.verifyEqual( double( B ), 1 : 0.5 : 3 );
            TestCase.verifyTrue( isa( B, 'OctoDoubleSlow' ) );
        end

        function TestConv( TestCase )
            % Bug #4: conv crashed due to u.Dot call
            U = OctoDoubleSlow( [ 1, 2, 3 ] );
            V = OctoDoubleSlow( [ 1, 1 ] );
            W = conv( U, V );
