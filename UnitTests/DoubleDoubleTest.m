classdef DoubleDoubleTest < matlab.unittest.TestCase
    ...
    properties
        ...
        Tol = 1e-30;  % Absolute tolerance for comparisons  % Relative tolerance for comparisons
    ...
    methods ( Test )
        ...
        function TestColonMixedTypes( TestCase )
            ...
            A = DoubleDouble( 1 ) : 5;
            TestCase.verifyEqual( double( A ), 1 : 5 );
            TestCase.verifyTrue( isa( A, 'DoubleDouble' ) );

            B = DoubleDouble( 1 ) : 0.5 : 3;
            TestCase.verifyEqual( double( B ), 1 : 0.5 : 3 );
            TestCase.verifyTrue( isa( B, 'DoubleDouble' ) );
        end

        function TestConv( TestCase )
            % Bug #4: conv crashed due to u.Dot call
            U = DoubleDouble( [ 1, 2, 3 ] );
            V = DoubleDouble( [ 1, 1 ] );
            W = conv( U, V );
