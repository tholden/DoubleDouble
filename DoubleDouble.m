classdef DoubleDouble < ED.BaseDoubleDouble & ED.ExtDouble
    ...
    properties ( Constant, GetAccess = public )
        ...
        log2  = DoubleDouble.MakeStatic( 6.931471805599452862e-01,  2.319046813846299558e-17 );
        log10 = DoubleDouble.MakeStatic( 2.302585092994045901e+00, -2.170756223382249351e-16 );
    ...
    methods
        ...
        function [ a1, a2 ] = Split( a )
            ...
            if isreal( a )
                Select = ( a > 9.9792015476735990583e+291 ) | ( a < -9.9792015476735990583e+291 ); % 2^970
                a = Assign( a, TimesPowerOf2( Index( a, Select ), 5.5511151231257827021e-17 ), Select ); % 2^( -54 )
                t1 = 9007199254740993.0 * a; % 2^53 + 1
                t2 = t1 - a;
                a1 = t1 - t2;
                a2 = a - a1;
                a1 = Assign( a1, TimesPowerOf2( Index( a1, Select ), 18014398509481984.0 ), Select ); % 2^54
                a2 = Assign( a2, TimesPowerOf2( Index( a2, Select ), 18014398509481984.0 ), Select ); % 2^54
            else
                [ r1, r2 ] = Split( real( a ) );
                [ i1, i2 ] = Split( imag( a ) );
                a1 = complex( r1, i1 );
