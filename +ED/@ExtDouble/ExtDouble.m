classdef ( Abstract ) ExtDouble < ED.BaseExtDoubleProperties
    ...
    methods ( Sealed )
        ...
        function v = Assign( v, Value, varargin )
            ...
            if ~all( v.v2 == 0, 'all' ) || ~all( Value.v2 == 0, 'all' )
                v = FillV2( v );
                Value = FillV2( Value );
        ...
        function v = floor( v )
            ...
            if all( v.v2 == 0, 'all' )
                ...
                if isempty( Select )
                    ...
                    x2 = zeros( size( x1 ), 'like', x1 );
                    x2 = Assign( x2, floor( Index( v.v2, Select ) ), Select );
                end
            end
            v = v.Make( x1, x2 );
            v = Normalize( v );
        ...
        function v = ceil( v )
            ...
            if all( v.v2 == 0, 'all' )
                ...
                if isempty( Select )
                    ...
                    x2 = zeros( size( x1 ), 'like', x1 );
                    x2 = Assign( x2, ceil( Index( v.v2, Select ) ), Select );
                end
            end
            v = v.Make( x1, x2 );
            v = Normalize( v );
        ...
        function v = fix( v )
            ...
            if all( v.v2 == 0, 'all' )
                ...
                if isempty( Select )
                    ...
                    x2 = zeros( size( x1 ), 'like', x1 );
                    x2 = Assign( x2, fix( Index( v.v2, Select ) ), Select );
                end
            end
            v = v.Make( x1, x2 );
            v = Normalize( v );
        ...
        function v = round( v )
            ...
            if all( v.v2 == 0, 'all' )
                ...
                if isempty( Select )
                    ...
                    x2 = zeros( size( x1 ), 'like', x1 );
                    x2 = Assign( x2, round( Index( v.v2, Select ) ), Select );
                    Select = find( ( abs( x1 - v.v1 ) == 0.5 ) & ( v.v2 < 0 ) );
        ...
        function [ C, ia, ic ] = unique( A, varargin )
            ...
            if all( A.v2 == 0, 'all' )
                ...
                C = AOriginal.Make( C, 0 );
            else
                A = [ A.v1, A.v2 ];
                [ C, ia, ic ] = unique( A, 'rows', varargin{:} );
                n = Size( 2 );
                C = AOriginal.Make( Index( C, ':', 1:n ), Index( C, ':', ( n + 1 ) : ( 2 * n ) ) );
            end
            if RowFlag
                C = C.';
            end
        ...
        function v = pow2( v, b )
            ...
            if isa( v, 'ED.ExtDouble' )
                ...
                v = b.Promote( pow2( v, double( b ) ) );
            end
        end

        function v = TimesPowerOf2( v, b )
            if isa( v, 'ED.ExtDouble' )
                v.v1 = TimesPowerOf2( v.v1, double( b ) );
                if ~all( v.v2 == 0, 'all' )
                    v.v2 = TimesPowerOf2( v.v2, double( b ) );
                end
            else
                v = b.Promote( TimesPowerOf2( v, double( b ) ) );
            end
        end
